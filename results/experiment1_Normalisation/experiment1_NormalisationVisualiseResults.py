...
# Load results
results = np.load("results/experiment1_Normalisation.npz", allow_pickle=True)
results = {k: v.item() for k, v in results.items()}  # convert back to dict

saveDir = "results/experiment1_Normalisation/graphs"
...
for normName in results.keys():
    ...
    converged = all([all(run) for run in results[normName]["converged"]])
