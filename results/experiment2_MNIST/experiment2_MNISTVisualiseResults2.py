...
fig.suptitle('Variance Across Runs', fontsize=18, fontweight='bold')
...
bars2 = ax1.bar(x_pos + width/2, variance_df["Loss Std"] * 20, width,  # Scale loss for visibility
                label='Loss Std (x20)', color='#e74c3c', alpha=0.7, edgecolor='black')

ax1.set_xlabel('Activation Function', fontsize=12)
ax1.set_ylabel('Standard Deviation', fontsize=12)
ax1.set_title('Standard Deviation Across Runs', fontsize=14, fontweight='bold')
...
ax2.set_title('Variability Range', fontsize=14, fontweight='bold')
...
fig.suptitle('Individual Runs', fontsize=16, fontweight='bold')
...
for activation in results.keys():
    ...
    # For each epoch, calculate std and mean variation
    cv_per_epoch = []
...
ax.set_title('Training Consistency', fontsize=14, fontweight='bold')
...
summary = summary.sort_values('Stability Score', ascending=True)  # Most stable first

# Save summary
summary.to_csv(f"{saveDir}_stability_summary.csv", index=False)
